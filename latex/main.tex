...
% ============================================================================
\begin{abstract}
This paper presents an end-to-end machine learning pipeline for credit risk scoring on the Home Credit Default Risk dataset comprising 307,511 loan applications. We engineer 145 raw features from seven relational tables and train three tree-based models---XGBoost, LightGBM, and CatBoost---using temporal cross-validation compliant with Basel III CRE36.54. The production CatBoost model achieves a Gini coefficient of 0.5814 and KS statistic of 0.4259 on an out-of-time test set. We validate fairness across demographic groups and provide SHAP-based explainability for adverse action notices per GDPR Article 22. The report positions the approach within the Basel IRB framework and documents deployment as a FastAPI scoring endpoint and Streamlit dashboard.
\end{abstract}
...
This paper addresses the challenge of developing a calibrated probability of default (PD) model on a multi-table relational dataset with severe class imbalance (approximately 8\% default rate). We employ a raw feature engineering strategy (bypassing WoE discretization) to preserve the continuous signal advantageous to tree-based learners. Three independent model architectures are trained and compared; the best single model (CatBoost, OOT Gini = 0.5814) is selected for production deployment and paired with SHAP-based explainability to meet emerging regulatory requirements for algorithmic transparency (EU AI Act Article 6, GDPR Article 22).

Our contribution is twofold: (1) a methodologically sound pipeline compliant with Basel CRE36.54 temporal validation, demonstrating the feasibility of ML-based PD models in a regulated environment, and (2) a deployed production system combining a calibrated point estimate with per-applicant SHAP attribution and fairness auditing across protected attributes (gender, age).
...
The ascendance of gradient boosted trees in machine learning has prompted exploration of these methods in credit applications. Al Daoud \cite{aldaoud2019} provides a direct comparison of XGBoost, LightGBM, and CatBoost on the Home Credit Default Risk dataset used in this work, finding CatBoost achieved marginally superior AUC but concluding that all three are viable for production credit models. Subsequent studies have demonstrated that raw (non-discretized) features yield superior tree model performance compared to WoE-encoded inputs \cite{xia2020, liu2023}, motivating our choice of continuous feature representations.

Probability calibration has received renewed attention in imbalanced classification. Niculescu-Mizil and Caruana \cite{niculescu2005predicting} established Platt scaling as a lightweight calibration method suitable for post-hoc model refinement, a pattern we adopt here. For handling severe class imbalance, synthetic minority oversampling (SMOTE) \cite{chawla2002smote} and cost-sensitive weighting remain standard; we employ the latter due to its superior KS performance on held-out sets.
...
[Appendix B: Model-specific ROC/PR curves for XGBoost and LightGBM, individual SHAP force plots, additional fairness cohort analysis]
