...
The production CatBoost model was audited for fairness across gender and age cohorts. The Disparate Impact Ratio for the protected attribute (gender) is 0.955, comfortably exceeding the 80\% legal threshold. Age is monitored only; age-related features were excluded from model training to avoid direct discrimination, aligning with GDPR and EU AI Act high-risk AI classification requirements.

\subsection{SHAP-Based Adverse Action Notices}

For each prediction, the model generates a ranked list of top-5 SHAP values explaining the most influential features. These are surfaced in the production FastAPI endpoint to enable GDPR Article 22 adverse action notice compliance.
