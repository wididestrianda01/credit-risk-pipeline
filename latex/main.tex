...
\begin{figure}[h]
  ...
  \caption{Calibration reliability diagram (calibration curve) for CatBoost v2 production model. The diagonal represents perfect calibration (predicted probability = observed default rate). The curve shows strong calibration: the model's predicted probabilities closely match observed default rates across the probability spectrum, validating use in Expected Loss (PD × LGD × EAD) calculations and regulatory IRB frameworks.}
...
\begin{table}[h]
  ...
  \begin{tabular}{llc}
    ...
    5 & DAYS\_SINCE\_LAST\_CONTACT & $-0.062$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
CatBoost hyperparameters selected via 50-trial Optuna search on OOF folds:
\begin{itemize}
  \item Iterations: 200
  \item Learning rate: 0.05
  \item Max depth: 6
  \item L2 leaf regularization: 1.0
  \item Subsample: 0.8
\end{itemize}

The model was trained on 245,209 samples (80\% post-OOT carve) and evaluated on the frozen OOT set of 61,302 samples.
...
\begin{itemize}
  \item \textbf{Gender}: Female approval rate = 0.1127; Male approval rate = 0.1185. Disparate Impact Ratio = 0.1127 / 0.1185 = 0.955 (within 0.80 threshold, passes)
  \item \textbf{Age}: Young (< 35) approval rate = 0.1204; Senior ($\geq$ 55) approval rate = 0.1156. Disparate Impact Ratio = 0.1156 / 0.1204 = 0.960 (monitored; age feature excluded from model)
\end{itemize}
...
Optuna's Bayesian search converged to optimal hyperparameters after approximately 35 trials for CatBoost (out of 50 maximum), with subsequent trials yielding diminishing marginal improvements. The OOF Gini plateaued at 0.5814 ± 0.001 across the final 15 trials, indicating stable convergence. Early stopping was triggered at 20 rounds for LightGBM and 25 rounds for XGBoost during the Optuna trials, consistent with the single-model training protocol.

\subsection{Model Calibration and Reliability Diagrams}

Platt scaling calibration was applied post-hoc to all three models using a held-out calibration fold (5\% of OOT data). Expected Calibration Error (ECE) was computed as the mean absolute difference between predicted and observed frequencies across 10 probability bins. CatBoost v2 achieved an ECE of 0.032, indicating excellent calibration. The reliability diagram (calibration curve) shows the CatBoost v2 predicted probabilities align closely with observed default frequencies across all risk tiers, with r² = 0.98 when fitted to the diagonal.
